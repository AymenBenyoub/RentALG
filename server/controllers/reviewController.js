...
exports.getReviewsForUser = async (req, res) => {
  ...
  try {
    ...
    // Fetch all accommodations listed by the user
    const [accommodations] = await connection.query(
      "SELECT id FROM accommodations WHERE host_id = ?",
      [hostId]
    );

    // Extract accommodation IDs from the result
    const accommodationIds = accommodations.map(
      (accommodation) => accommodation.id
    );

    // Generate a comma-separated list of placeholders for the accommodation IDs
    const placeholders = Array(accommodationIds.length).fill("?").join(",");

    // Fetch reviews for the accommodations listed by the user
    const query = `SELECT * FROM reviews WHERE accommodation_id IN (${placeholders})`;
    const [reviews] = await connection.query(query, accommodationIds);
